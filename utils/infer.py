...
def infer_new_patient_fixed(
    ...
    if outcome_type == "pfs":
        ART['pp_train_medians'] = [
            'pp_pfs_train_medians.joblib',
            'pp_pfs_train_medians.pkl',
            'pp_pfs_train_medians.csv'
        ]
        ART['pooled_logit'] = [
            'pooled_logit_pfs_logreg_saga.joblib',
            'pooled_logit_pfs.joblib',
            'pooled_logit_pfs_logreg_saga.pkl'
        ]
        ART['model_columns'] = [
            'pooled_logit_pfs_model_columns.csv',
            'pooled_logit_pfs_model_columns.joblib'
        ]
        ART['pp_scaler'] = [
            'pp_pfs_scaler.joblib',
            'pp_pfs_scaler.pkl'
        ]
        # For now, we do NOT use a PFS causal forest for horizons in this function.
        # We'll simply skip CF CATEs when outcome_type == "pfs".
        # So we leave forests_bundle as is, but we won't use it.
    ...
    # ------------------ POOLED-LOGIT SURVIVAL (OS or PFS) ------------------
    survival_df = None
    if pooled_logit is None or model_columns is None:
        errors['pooled_logit'] = f"pooled-logit ({outcome_type}) or model_columns missing; cannot compute survival."
    else:
        try:
            ...
            if 'build_X_for_pp' in globals():
                ...
                if pp_train_medians is not None:
                    # if it's a dict or Series
                    if isinstance(pp_train_medians, dict):
                        items_iter = pp_train_medians.items()
                    elif isinstance(pp_train_medians, pd.Series):
                        items_iter = pp_train_medians.to_dict().items()
                    else:
                        try:
                            items_iter = pp_train_medians.items()
                        except Exception:
                    ...
                    for c, v in items_iter:
                        X_pp[c] = pd.to_numeric(
                            df_pp_new.get(c, pd.Series([np.nan]*len(df_pp_new))),
                            errors='coerce'
                        ).fillna(v)
            ...
            errors['pooled_logit'] = f"pipelined survival predict failed ({outcome_type}): {e}"

    # ------------------ CF CATE PREDICTIONS (OS ONLY) ------------------
    cate_results = {}

    if outcome_type == "pfs":
        # For now we do NOT expose PFS CATEs here. We only return survival curves.
        # This keeps the app logic simple: it will show "No CATE estimates" for PFS.
        pass
    else:
        # ---------- build Xpatient (for CF) ----------
        Xpatient = None
        if patient_columns is None:
            errors['patient_columns'] = (
                "patient_columns artifact missing. CF prediction will be impossible without "
                "canonical patient feature names."
            )
        else:
            try:
                ...
                if patient_scaler is not None:
                    try:
                        ...
                        errors['scaler'] = (
                            "scaler exists but failed to transform Xpatient; proceeding without scaling"
                        )
        ...
        try:
            cate_results = dict(
                sorted(
                    ...
                    key=lambda kv: (
                        float(kv[0])
                        if isinstance(kv[0], (int, float, str))
                        and str(kv[0]).replace('.', '', 1).isdigit()
                        else 1e9
                    )
                )
            )
